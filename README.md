...
**New in v1 — Low Volatility Signal:** a second, independent signal ranking stocks by rolling volatility, holding the 3 least volatile stocks each month instead of the highest momentum stocks. This is tested independently alongside momentum, before the two are properly combined into a composite ranking in v2.
...
## v1 Results (July 2021 – June 2026)

| Metric             | Strategy | Benchmark | Walk-Forward | Low Volatility |
|--------------------|----------|-----------|---------------|-----------------|
| Total Return        | 21.8%    | 153.6%    | 25.4%          | -2.8%            |
| Annualised Return    | 4.0%     | 20.5%     | 4.6%           | -0.6%            |
| Sharpe Ratio         | 1.08     | 1.14      | 1.81           | -0.19            |
| Max Drawdown          | -2.8%    | -25.9%    | -1.9%          | -8.1%            |
...
That said, the strategy still lags the benchmark's raw returns significantly. This is likely because momentum alone is a weak standalone signal, and a 20-stock universe with only one factor leaves a lot of alpha on the table compared to simply holding the market.

**Walk-Forward Observations**

The walk-forward out-of-sample results are the more interesting finding here. Despite only having access to information available at each point in time, the walk-forward strategy actually achieved a higher Sharpe ratio (1.81) than both the in-sample strategy (1.08) and the benchmark (1.14), alongside the lowest max drawdown of the three (-1.9%). This suggests the momentum signal is not simply overfitted to one fixed historical period, it holds up reasonably well when forced to make decisions using only past information, which is a genuinely encouraging sign for the strategy's robustness.

**Low Volatility Observations**

The low volatility signal, tested on its own, performed poorly across every metric, a negative total return (-2.8%), a negative Sharpe ratio (-0.19), and the second-worst max drawdown of the four results (-8.1%). Unlike momentum, holding the least volatile stocks each month did not generate positive risk-adjusted returns in isolation over this period.

A likely explanation is that low volatility on its own is not a return-generating signal, because it's a risk-reducing one. Stocks with low rolling volatility are not necessarily good investments, they are simply stable ones, and stability alone doesn't guarantee positive performance, particularly in a period where the broader benchmark was driven by strong growth in a small number of higher-volatility stocks. The 3-month rolling window may also be too short to estimate volatility reliably, introducing noise into which stocks get selected each month.

This result doesn't suggest low volatility is a bad concept, but it does suggest it isn't useful as a standalone strategy in this universe and timeframe. It reinforces why low volatility is typically combined with a genuine return-generating signal like momentum, rather than used alone, and makes the v2 composite signal a genuinely important next test rather than an assumed improvement.
...
- Combine momentum and low volatility into a proper composite signal
- Add a value factor
- Run factor regressions and information coefficient analysis once ST120-10 Introduction to Probability is complete in my first year of university